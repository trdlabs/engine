...
//
// Исход РАЗЛИЧИМ: филл либо снятие с названной причиной. Нулевой филл (сокращать нечего) вернуть
// нельзя — `applyFill` отвергает `qty <= 0` броском, и разбираться в этом пришлось бы вызывающему.
//
// `shiftBps` и `sizeAtShiftedPrice` остаются публичными для гейтов и путей без клампа; цену
// исполнения хост берёт из результата `executeFill`, а не считает её вторым вызовом.
export {
  executeFill,
  shiftBps,
  sizeAtShiftedPrice,
