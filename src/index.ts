...
// ВТОРАЯ ФАЗА МАТЧИНГА. `matchBar` выше отвечает «какая заявка и по какой цене»; размер в базовой
// валюте появляется только здесь — нотионал, делённый на цену ПОСЛЕ сдвига на проскальзывание,
// одним десятичным выражением. Обе функции экспортируются вместе, потому что порознь они не
// употребляются: хост, у которого есть только первая, вынужден выдумывать размер (см. шапку
// `sim-exchange.ts`), а хост, у которого есть только вторая, не знает цены.
// ВТОРАЯ ФАЗА — ОДНОЙ ОПЕРАЦИЕЙ. `executeFill` берёт цену матча и отдаёт готовое исполнение:
// размер, нотионал, комиссию и факт клампа. Набор примитивов вместо неё оставлял бы потребителю
// программу из трёх решений (был ли кламп, какой нотионал публиковать, от чего считать комиссию) —
// а развилка у потребителя означает, что два хоста разойдутся в деньгах и молча.
//
// `shiftBps` остаётся публичной: цена исполнения нужна хосту для записи филла, и брать её надо той
// же арифметикой. `sizeAtShiftedPrice` — для путей без клампа и для проверки самих гейтов.
export { executeFill, shiftBps, sizeAtShiftedPrice, type ExecutedFill } from './core/money.js';
