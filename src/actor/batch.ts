...
export function deepFreeze<T>(value: T, seen = new WeakSet<object>()): T {
  ...
  // УЖЕ ЗАМОРОЖЕННОЕ ПОДДЕРЕВО ПРОПУСКАЕТСЯ ЦЕЛИКОМ. Это не микрооптимизация, а разница между
  // линейной и квадратичной сложностью прогона.
  //
  ...
  //
  // Пропуск корректен, а не «достаточно хорош»: заморозить объект мог только этот же обход, а он
  // всегда спускается к детям ДО `Object.freeze` родителя. Значит замороженный родитель
  // гарантирует замороженных детей. Единственный способ нарушить это — заморозить объект вручную
  // снаружи, минуя обход; ровно поэтому функция экспортируется, а не оставлена приватной: у
  // правила должно быть одно место, а не два.
