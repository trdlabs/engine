 ...
 */
/**
 * Реестр поддеревьев, замороженных ИМЕННО ЭТИМ обходом.
 *
 * WeakSet не итерируется и в выдачу не попадает — детерминизма он не касается. Глобальность
 * безопасна, потому что «глубоко заморожено» необратимо: разморозить объект нельзя, значит запись
 * не может протухнуть.
 */
const DEEPLY_FROZEN = new WeakSet<object>();

export function deepFreeze<T>(value: T, seen = new WeakSet<object>()): T {
  if (typeof value !== 'object' || value === null) return value;
  const obj = value as unknown as object;

  // УЖЕ ЗАМОРОЖЕННОЕ ЭТИМ ОБХОДОМ ПОДДЕРЕВО ПРОПУСКАЕТСЯ ЦЕЛИКОМ. Это не микрооптимизация, а
  // разница между линейной и квадратичной сложностью прогона.
  //
  // Замер (S2, мидгейт): без этой ветки обход стоил 20.25 мкс на вызов и выполнялся пять раз на
  // батч из четырёх команд — ≈101 мкс из 86–125 мкс всей стоимости диспатча. То есть заморозка,
  // добавленная ради атомарности, СОСТАВЛЯЛА почти всю цену, и цена росла вместе с состоянием:
  // ордера, таймеры и филлы накапливаются, а обход шёл по всему состоянию на КАЖДУЮ команду.
  //
  // ПОЧЕМУ НЕ `Object.isFrozen`. Первая редакция ветки спрашивала именно его и обосновывала так:
  // «заморозить объект мог только этот же обход, а он спускается к детям ДО `Object.freeze`
  // родителя». Обоснование неверно. `Object.isFrozen` истинен и для ПОВЕРХНОСТНО замороженного
  // объекта, а такие в состоянии есть: любой `Object.freeze` в чужом модуле, любая замороженная
  // константа, приехавшая в состояние по ссылке. Для такого родителя обход возвращался
  // немедленно, и его ИЗМЕНЯЕМЫЕ дети оставались изменяемыми — дыра приходилась ровно на то
  // место, ради которого заморозка заведена. Гарантия «частичных эффектов нет» переставала быть
  // структурной там, где состояние частично заморожено снаружи, то есть молча.
  //
  // Реестр отвечает на другой вопрос: не «заморожен ли объект», а «замораживал ли его ЭТОТ
  // обход». Только на второй вопрос ответ «да» означает замороженное поддерево.
  if (DEEPLY_FROZEN.has(obj)) return value;

  if (seen.has(obj)) return value;
  seen.add(obj);
  // ГРАНИЦА ОБХОДА названа явно: собственные перечислимые строковые ключи. Символьные и
  // неперечислимые свойства не обходятся — сам объект `Object.freeze` запирает целиком, но
  // вложенное под такими ключами остаётся изменяемым. Для состояния ядра это не дыра, а его
  // определение: `Checkpoint` — простые данные, переживающие каноническое кодирование, а оно
  // символьных ключей не видит вовсе.
  for (const key of Object.keys(obj).sort()) {
    deepFreeze((obj as Record<string, unknown>)[key], seen);
  }
  Object.freeze(value);
  DEEPLY_FROZEN.add(obj);
  return value;
}
